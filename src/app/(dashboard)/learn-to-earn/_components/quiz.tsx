...
export default function Quiz({
  ...
  return (
    <div className="border border-white/10 rounded-[20px] py-6 px-4 md:px-6 flex flex-col items-center gap-8 md:gap-10">
      <div className="flex flex-col gap-4 w-full">
        <div className="flex items-center justify-between">
          <p className="text-sm text-white/50">1/1</p>
          <button className="text-underline font-semibold ttext-xs md:ext-sm text-white/50">
            Skip quiz
          </button>
        </div>
      ...
      <div className="space-y-4 md:space-y-8 w-full">
        {quizFormQuestions[0].options.map((option, i) => (
          <button
            ...
            } text-white/50 tracking-[2px] text-start text-sm md:text-base leading-[150%] py-4 px-4 md:px-6 flex items-center gap-4 disabled:opacity-100!`}
            onClick={() => {
              setQuizForm((prev) => ({
                ...prev,
                [0]: i,
      ...
      {quizResults[0] === null && (
        <button
          ...
          } text-[#181812B2] text-base font-bold flex items-center justify-center shadow-[inset_4px_3px_2px_0px_#EDEBB680] border border-[#ACA40F80] bg-[#BDB510] rounded-[15px] h-[55px] w-[260px] md:w-[214px]`}
        >
          Check
        </button>
      )}
      ...
      {quizResults[0] !== null && (
        <>
          <p className="text-sm md:text-base w-full font-semibold text-[#BEBBC6]">
          ...
          <button
            ...
            } text-base font-bold flex items-center justify-center rounded-[10px] h-[60px] w-[260px] md:w-[214px]`}
          >
            Next
          </button>
        </>
